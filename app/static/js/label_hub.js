(function () {
  ...
  function renderHistoricalRoleHistory(history, stockoutEvent) {
    ...
    var timelineHtml = nodes.slice().reverse().map(function (node) {
      ...
      var inventoryMissing = node.sellable_inventory === null || node.sellable_inventory === undefined || node.sellable_inventory === "";
      var inventoryNumber = Number(node.sellable_inventory);
      var inventoryText = inventoryMissing ? "可售库存：缺失" : "可售库存：" + evidenceNumber(node.sellable_inventory, 0);
      var inventoryClass = inventoryMissing ? " is-missing" : (Number.isFinite(inventoryNumber) && inventoryNumber <= 0 ? " is-zero" : "");
      return '<li class="is-' + app.escapeHtml(node.role || "unavailable") + (node.is_valid ? '' : ' is-unavailable') + (node.is_pre_oos_source ? ' is-pre-oos-source' : '') + '" title="' + app.escapeHtml(reasonTitle) + '"><time datetime="' + app.escapeHtml(endDate) + '">' + app.escapeHtml(shortDate) + '</time><b>' + app.escapeHtml(nodeLabel) + '</b><small>' + app.escapeHtml(evidenceText) + '</small><span class="label-hub-stockout-history-inventory' + inventoryClass + '">' + app.escapeHtml(inventoryText) + '</span>' + (node.is_pre_oos_source ? '<em>断货前取值</em>' : '') + '</li>';
    }).join("");
    stockoutEvent = stockoutEvent || {};
    var eventKind = stockoutEvent.date_kind || "pending";
    var eventInventoryMissing = stockoutEvent.sellable_inventory === null || stockoutEvent.sellable_inventory === undefined || stockoutEvent.sellable_inventory === "";
    var eventInventoryNumber = Number(stockoutEvent.sellable_inventory);
    var eventInventoryText = eventInventoryMissing ? "可售库存：缺失" : "可售库存：" + evidenceNumber(stockoutEvent.sellable_inventory, 0);
    var eventInventoryClass = eventInventoryMissing ? " is-missing" : (Number.isFinite(eventInventoryNumber) && eventInventoryNumber <= 0 ? " is-zero" : "");
    var eventHtml = '<li class="label-hub-stockout-history-event is-' + app.escapeHtml(eventKind) + '"><time>' + app.escapeHtml(stockoutEvent.date || "--") + '</time><b>断货事件</b><small>' + app.escapeHtml(stockoutEvent.date_label || stockoutEventDateLabel(eventKind)) + '</small><span class="label-hub-stockout-history-inventory' + eventInventoryClass + '">' + app.escapeHtml(eventInventoryText) + '</span></li>';
    return '<div class="label-hub-stockout-history-layout"><section class="label-hub-stockout-history-summary"><div><span>历史主导角色</span><strong>' + app.escapeHtml(dominant.label || "暂无") + '</strong><small>' + formatNumber(dominant.count || 0) + ' / ' + formatNumber(history.valid_node_count || 0) + ' 个有效节点 · ' + formatPercent(dominant.share || 0) + '</small></div><div><span>历史节点覆盖</span><strong>' + formatNumber(history.valid_node_count || 0) + ' 个参与稳定性</strong><small>' + formatNumber(history.calculated_node_count || 0) + ' 个正常计算 · ' + formatNumber(history.carried_node_count || 0) + ' 个沿用 · ' + formatNumber(history.recovery_node_count || 0) + ' 个恢复观察 · ' + formatNumber(history.unavailable_node_count || 0) + ' 个真正不可判</small></div></section><p class="label-hub-stockout-history-status is-' + app.escapeHtml(history.status || "missing") + '">' + app.escapeHtml(statusCopy) + '</p>' + (distributionHtml ? '<section class="label-hub-stockout-evidence-section"><div class="label-hub-stockout-evidence-section-head"><div><span>角色占比</span><h3>历史窗口中分别处于什么角色</h3></div><p>滚动30天 · 每天一个节点 · 连续5天确认切换</p></div><div class="label-hub-stockout-history-distribution">' + distributionHtml + '</div></section>' : '') + '<section class="label-hub-stockout-evidence-section"><div class="label-hub-stockout-evidence-section-head"><div><span>角色变化</span><h3>断货前角色及历史轨迹</h3></div><p>最新在左，越靠左越接近断货；库存为 0 用于核对库存事实，断货结论仍以标签完整口径为准</p></div><div class="label-hub-stockout-history-timeline-wrap"><ol class="label-hub-stockout-history-timeline">' + eventHtml + timelineHtml + '</ol></div></section></div>';
  }
