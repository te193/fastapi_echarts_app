(function () {
  ...
  function renderRows(rows) {
    if (!rows.length) {
      elements.skuTableBody.innerHTML = '<tr><td colspan="16"><div class="empty-state">当前筛选条件下没有产品，请调整条件后再查看。</div></td></tr>';
      return;
    }
    ...
    elements.skuTableBody.innerHTML = rows.map(function (item) {
      return [
        ...
        "<td>" + app.formatCurrency(item.current_price) + "</td>",
        "<td>" + app.formatCurrency(item.limit_price) + "</td>",
        "<td>" + app.formatCurrency(item.price_gap) + "</td>",
  ...
  function openDrawer(itemId, trendDays) {
    app.apiGet("/api/detail/" + encodeURIComponent(itemId), { trend_days: trendDays }).then(function (payload) {
      ...
      elements.drawerContent.innerHTML = [
        ...
        info("当前售价", app.formatCurrency(payload.item.current_price)),
