...
RULE_VERSION = "stockout_historical_operating_v2_calendar_daily_20260824"
...
def find_current_oos_event(rows: Iterable[Mapping[str, Any]], *, current_date: date) -> dict[str, Any]:
    ordered = [row for row in _normalized_rows(rows) if row["dt_date"] <= current_date]
    current = next((row for row in reversed(ordered) if row["dt_date"] == current_date), None)
    base = {
        "current_oos_flag": bool(current and current["fba_available"] == 0),
        "oos_start_date": None,
        "oos_start_method": "latest_recovery_gt5_then_first_zero",
    ...
    recovery_index = next(
        (index for index in range(len(ordered) - 1, -1, -1) if ordered[index]["fba_available"] is not None and ordered[index]["fba_available"] > 5),
        None,
    )
    if recovery_index is None:
        observed = next((row["dt_date"] for row in ordered if row["fba_available"] == 0), current_date)
        base.update(
            oos_start_confidence="left_boundary_incomplete",
            observed_oos_since_date=observed,
            minimum_current_oos_days=(current_date - observed).days + 1,
            event_boundary_incomplete=True,
        )
        return base

    after_recovery = ordered[recovery_index + 1 :]
    first_zero_index = next((index for index, row in enumerate(after_recovery) if row["fba_available"] == 0), None)
    if first_zero_index is None:
        base["oos_start_confidence"] = "event_start_missing"
        base["event_boundary_incomplete"] = True
        return base
    first_zero = after_recovery[first_zero_index]
    event_rows = ordered[recovery_index:]
    gaps = _gap_ranges(
        event_rows,
        window_start=ordered[recovery_index]["dt_date"],
        window_end=current_date,
    )
    if gaps:
        base.update(
            oos_start_confidence="event_boundary_incomplete",
            observed_oos_since_date=first_zero["dt_date"],
            minimum_current_oos_days=(current_date - first_zero["dt_date"]).days + 1,
            event_boundary_incomplete=True,
        )
        return base

    previous = ordered[recovery_index - 1] if recovery_index > 0 else None
    one_day_recovery = bool(
        previous
        and previous["fba_available"] == 0
        and previous["dt_date"] + timedelta(days=1) == ordered[recovery_index]["dt_date"]
        and ordered[recovery_index]["dt_date"] + timedelta(days=1) == first_zero["dt_date"]
    )
    base.update(
        oos_start_date=first_zero["dt_date"],
...
def classify_msku_role(daily_sales: float, margin_rate: float | None) -> str:
    ...
    if margin_rate is None or margin_rate < 0.05:
        return "problem"
    if daily_sales >= 5 and margin_rate >= 0.15:
        return "star"
    if 1 <= daily_sales < 5 and margin_rate >= 0.25:
        return "star"
    if daily_sales >= 5 and margin_rate >= 0.05:
        return "potential"
    if 1 <= daily_sales < 5 and margin_rate >= 0.10:
        return "potential"
    if daily_sales > 0:
        return "dog"
    return "problem"
...
def evaluate_stockout_history(
    ...
    *,
    ...
    event = find_current_oos_event(ordered, current_date=current_date)
    result: dict[str, Any] = {
        **event,
