-- ============================================================================
-- 20260728返场规则对齐正式版
-- 为兼容远端现有任务，过程名继续使用 sp_dws_标签表_一体化_v45。
-- 本版仅调整返厂品识别、恢复率、运营状态返厂品及返厂品阶段下钻四阶段；
-- 生命周期、销售角色及其他标签逻辑和统计周期保持不变。
--
-- 返场规则对齐变更说明：
--   1. 返厂品要求最近180天出现过销量>0，并识别最新一轮“首次库存=0 -> 后续首次库存>5”的完整返场事件；
--   2. 返场D1-D7观察期、D8-D21运营干预期、D22以后持续干预或达标退出；达标退出不计入运营状态返厂品；
--      已进入返场流程的MSKU即使再次FBA可售为0，仍优先标记为返厂品，不降级为断货中或停售；
--   3. 恢复率按讨论稿口径计算；生命周期、销售角色及其他标签逻辑不变；主表仍只保留当前业务日期及前一天；
--   4. 执行本文件时，自动创建或更新 evt_标签表，使其每天 07:20 调用 V45；无需再人工 ALTER EVENT；
--   5. 不删除、不替换 V34/V35/V36/V37/V38 的过程，也不删除标签数据。
--
-- 唯一对外过程：
--   CALL dws_datasync.sp_dws_标签表_一体化_v45('YYYY-MM-DD');  -- 手工回补/重算某一天
--   CALL dws_datasync.sp_dws_标签表_一体化_v45(NULL);          -- 计算产品表现表最新业务日期（日常事件使用）
-- ============================================================================

-- 仅删除并重建同名 V45 过程；旧版本过程保持不变。
DROP PROCEDURE IF EXISTS `dws_datasync`.`sp_dws_标签表_一体化_v45`;

-- V35/V36/V37/V38 未曾部署时才新增字段；字段已存在时不改动 dws_标签表结构。
SET @evidence_json_column_exists := (
    SELECT COUNT(*)
    FROM information_schema.COLUMNS
    WHERE TABLE_SCHEMA = 'dws_datasync'
      AND TABLE_NAME = 'dws_标签表'
      AND COLUMN_NAME = 'evidence_json'
);
SET @evidence_json_alter_sql := IF(
    @evidence_json_column_exists = 0,
    'ALTER TABLE dws_datasync.dws_标签表 ADD COLUMN evidence_json JSON NULL COMMENT ''本次打标实际使用的指标、统计窗口和命中阈值''',
    'SELECT ''evidence_json already exists'' AS message'
);
PREPARE stmt_evidence_json FROM @evidence_json_alter_sql;
EXECUTE stmt_evidence_json;
DEALLOCATE PREPARE stmt_evidence_json;

-- V45 成熟期不可回退方案：不新建、不读取 dws_生命周期成熟期历史。
-- 每日读取 dws_标签表中当前业务日期之前最近一个业务日期的成熟期标签，并通过 evidence_json 逐日续传首次成熟日期。

-- V45返厂品阶段下钻标签字典：仅同步子标签名称、ID对应和tag_rule，其他字段不变。
START TRANSACTION;

UPDATE `dws_datasync`.`dws_标签详情表`
SET `sub_label_name` = '观察期',
    `tag_rule` = '返场D1-D7'
WHERE `label_name` = '返厂品阶段下钻'
  AND `sub_label_id` = 501;

UPDATE `dws_datasync`.`dws_标签详情表`
SET `sub_label_name` = '运营干预期',
    `tag_rule` = '返场D8-D21，即使期间恢复率达到70%，仍观察至D21结束'
WHERE `label_name` = '返厂品阶段下钻'
  AND `sub_label_id` = 502;

UPDATE `dws_datasync`.`dws_标签详情表`
SET `sub_label_name` = '持续干预期',
    `tag_rule` = 'D21恢复率低于70%或为空，从D22起继续跟踪，仍然打标为持续干预期'
WHERE `label_name` = '返厂品阶段下钻'
  AND `sub_label_id` = 503;

UPDATE `dws_datasync`.`dws_标签详情表`
SET `sub_label_name` = '达标退出',
    `tag_rule` = 'D21恢复率达到70%的，于D22退出；D21未达标的，D22以后累计平均恢复率首次达到70%的当天退出'
WHERE `label_name` = '返厂品阶段下钻'
  AND `sub_label_id` = 504;

INSERT INTO `dws_datasync`.`dws_标签详情表` (
...
SELECT
    src.`label_id`, src.`label_name`, 504, '达标退出',
    'D21恢复率达到70%的，于D22退出；D21未达标的，D22以后累计平均恢复率首次达到70%的当天退出',
    src.`business_definition`, src.`business_owner`, src.`label_category`,
    src.`update_frequency`, src.`mutual_exclusion`, src.`status`,
    src.`create_time`, src.`tagging_method`
FROM `dws_datasync`.`dws_标签详情表` src
WHERE src.`label_name` = '返厂品阶段下钻'
  AND src.`sub_label_id` = 501
  AND NOT EXISTS (
      SELECT 1
      FROM `dws_datasync`.`dws_标签详情表` existing_row
      WHERE existing_row.`sub_label_id` = 504
  );
...
CREATE PROCEDURE `dws_datasync`.`sp_dws_标签表_一体化_v45`(IN p_data_date DATE)
BEGIN
    ...
    BEGIN
            ...
            IF v_data_date IS NOT NULL THEN
                ...
                -- 返厂品/返场事件 V46：先使用2026年至统计日的完整历史识别
                ...
                -- 页面返场池要求最近180天内实际出现过断货；更早的断货只作历史上下文，
                -- 不单独纳入当前统计范围。
                WHERE o.oos_start_date BETWEEN v_180d AND v_data_date
                GROUP BY o.country_category, o.store, o.MSKU, o.oos_start_date
                -- 先从完整历史计算真实首次恢复日，再判断该事件是否属于最近180天。
                -- 不可在MIN之前裁剪日期，否则窗口第一天可能被误判为返场开始日。
                ...
                -- 返厂品要求最近180天周期总销量>0，与返场页面准入条件一致。
                SET v_stage = 'tmp_return_sales_180d';
                DROP TEMPORARY TABLE IF EXISTS tmp_return_sales_180d;
                CREATE TEMPORARY TABLE tmp_return_sales_180d AS
                SELECT country_category,
                       store,
                       MSKU,
                        CASE
                            WHEN SUM(COALESCE(site_sales_volume, 0)) > 0 THEN 1
                            ELSE 0
                        END AS has_sales_180d
                FROM tmp_return_perf_daily
                WHERE stat_date BETWEEN v_180d AND v_data_date
                GROUP BY country_category, store, MSKU;
                ALTER TABLE tmp_return_sales_180d
                    ADD INDEX idx_tmp_return_sales_180d (country_category, store, MSKU, has_sales_180d);
                ...
                FROM (
                    ...
                    JOIN tmp_return_sales_180d s
                      ON s.country_category = c.country_category
                     AND s.store = c.store
                     AND s.MSKU = c.MSKU
                     AND s.has_sales_180d = 1
                ) latest_round
                WHERE rn = 1;
                ...
                SELECT v_data_date,
                                ...
                                CASE h.label_name
                                    WHEN '销售角色' THEN JSON_OBJECT(
                                        'schema_version','1.0','rule_version','v45','type','sales_role',
                                        'window',JSON_OBJECT(
    ...
    BEGIN
            ...
            IF v_data_date IS NOT NULL THEN
                ...
                SELECT v_data_date, r.country_category, r.country, r.store, r.msku,
                       ...
                       JSON_OBJECT(
                           'schema_version','1.0','rule_version','v45','type','station_sales_role',
                ...
                SET v_record_count = v_record_count + ROW_COUNT();                COMMIT;
...
-- 保持 V34 的部署方式：若事件不存在则创建，若已存在则直接切换调用目标。
-- 因此执行本文件后，无需再手工 ALTER EVENT。
...
-- ============================================================================
-- V45 首次执行与回补说明：首次请分开回补周末两个业务日期2026-07-25和2026-07-26。
-- 请按日期分开执行：先执行并核查2026-07-25，再执行2026-07-26。
--
-- CALL dws_datasync.sp_dws_标签表_一体化_v45('2026-07-25');
-- -- 核查生命周期、站点生命周期、evidence_json，以及从上一已存在业务日期续传的成熟期。
--
-- CALL dws_datasync.sp_dws_标签表_一体化_v45('2026-07-26');
--
-- 第二次成功后，dws_datasync.dws_标签表仅保留2026-07-25、2026-07-26两天数据。
-- 后续 evt_标签表 每天 07:20 自动调用 V45(NULL)，仍以产品表现表最新 start_date 为业务日期。
--
-- 本版本不创建、不读取 dws_生命周期成熟期历史；V42 已创建的同名历史表不在本脚本中删除，
-- 避免部署脚本误删既有数据，但 V45 不再依赖该表。
-- ============================================================================
