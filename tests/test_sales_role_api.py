...
class FakeSalesRoleService:
    ...
    def get_sales_role_lifecycle_payload(self, **kwargs):
        self.calls.append({"method": "lifecycle", **kwargs})
        return {"ok": True, "rows": [{"lifecycle_label": "新品期", "sales_role": "潜力产品"}]}

    def get_sales_role_lifecycle_export_payload(self, **kwargs):
        self.calls.append({"method": "lifecycle_export", **kwargs})
        return {
            "rows": [
                {
                    "lifecycle_label": "新品期",
...
class SalesRoleApiTests(unittest.TestCase):
    ...
    def test_sales_role_lifecycle_payload_passes_query_params_to_service(self):
        service = FakeSalesRoleService()

        with patch("app.main.dashboard_service", service):
            payload = main.api_sales_role_lifecycle(
                period="90d",
                country_category="欧洲站",
                seller_name_new="StoreA",
                lifecycle_label="202",
                sales_role="potential",
                keyword="MSKU1",
                page=2,
                page_size=50,
                sort_field="order_gross_margin",
                sort_dir="asc",
            )
    ...
    def test_sales_role_lifecycle_export_returns_csv(self):
        service = FakeSalesRoleService()

        with patch("app.main.dashboard_service", service):
            response = main.api_sales_role_lifecycle_export(
                period="30d",
                country_category="欧洲站",
                lifecycle_label="202",
                sales_role="potential",
