...
create table if not exists etl_datasync.dashboard_sales_role_period_snapshot (
    ...
    ending_inventory_qty decimal(18,4) not null default 0 comment '周期末FBA可售库存，取period_end当天汇总',
...
from (
    ...
    from (
        select
            ...
            sum(case when p.dt_date = %(period_end)s then p.afn_fulfillable_quantity else 0 end) as ending_inventory_qty,
