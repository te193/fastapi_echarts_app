...
def connect_target():
    return connect_with_retry(
        "target",
        host=cfg.DASHBOARD_DB_HOST,
        port=cfg.DASHBOARD_DB_PORT,
        user=cfg.DASHBOARD_DB_USER,
        password=cfg.DASHBOARD_DB_PASSWORD,
        database=cfg.DASHBOARD_DB_NAME,
        charset=cfg.DASHBOARD_DB_CHARSET,
...
def connect_source():
    return connect_with_retry(
        "source",
        host=cfg.DASHBOARD_SOURCE_DB_HOST,
        port=cfg.DASHBOARD_SOURCE_DB_PORT,
        user=cfg.DASHBOARD_SOURCE_DB_USER,
        password=cfg.DASHBOARD_SOURCE_DB_PASSWORD,
        database=cfg.DASHBOARD_SOURCE_DB_NAME,
        charset=cfg.DASHBOARD_SOURCE_DB_CHARSET,
...
def build_schema_config() -> SchemaConfig:
    return SchemaConfig(
        target_schema=clean_identifier(cfg.DASHBOARD_TARGET_SCHEMA, "etl_datasync"),
        etl_source_schema=clean_identifier(cfg.DASHBOARD_ETL_SOURCE_SCHEMA, "etl_datasync"),
        dwd_source_schema=clean_identifier(cfg.DASHBOARD_DWD_SOURCE_SCHEMA, "dwd_datasync"),
        pricing_source_schema=clean_identifier(cfg.DASHBOARD_PRICING_SOURCE_SCHEMA, "temporary_APP"),
    )
...
create table if not exists etl_datasync.dashboard_limit_price_daily_snapshot (
    ...
    tax_inclusive_price decimal(18,4) null,
    tax_inclusive_price_noad decimal(18,4) null,
    tax_inclusive_price_adj decimal(18,4) null,
    created_at datetime not null default current_timestamp,
    updated_at datetime not null default current_timestamp on update current_timestamp,
    unique key uk_snapshot_item (snapshot_date, item_key),
    key idx_snapshot_store (snapshot_date, seller_name_new),
...
create table if not exists etl_datasync.dashboard_product_period_snapshot (
    ...
    limit_price_adj decimal(18,4) null,
    over_limit_flag tinyint not null default 0,
    shipping_method varchar(64) null,
    target_margin decimal(10,4) null,
    limit_price_without_ad decimal(18,4) null,
...
where date(l.create_time) = %(snapshot_date)s
group by
    date(l.create_time),
    l.country_category,
    l.marketplace,
...
select
    ...
    max(tax_inclusive_price) as tax_inclusive_price,
    max(tax_inclusive_price_noad) as tax_inclusive_price_noad,
    max(tax_inclusive_price_adj) as tax_inclusive_price_adj,
    now() as created_at,
    now() as updated_at
...
insert into etl_datasync.dashboard_product_period_snapshot (
    ...
    current_price_cny, price_currency, current_price, limit_price, limit_price_adj,
...
with product_period as (
    select
        ...
        round(sum(order_gross_profit) / nullif(sum(sales_amount_ex_tax), 0), 4) as order_gross_margin,
...
COLUMN_COMMENTS = {
    ...
    "tax_inclusive_price_noad": "不含广告含税限价",
    "tax_inclusive_price_adj": "调整后含税限价",
    "stat_period": "统计周期",
    "margin_band": "毛利率分层",
    "filter_flag": "看板筛选标记",
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "Available: product_performance_daily, restock_snapshot, inventory_snapshot, "
