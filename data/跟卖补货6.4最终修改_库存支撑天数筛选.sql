...
# salable days preview only; do not insert into etl_datasync.pur_plan_prod_perf_salable_days_stat in this version
...
select pp.country_category,
       pp.seller_name_new,
       pp.seller_sku_adj,
       count(distinct case
                          when pp.start_date >= curdate() - interval 90 day
                              and pp.afn_fulfillable_quantity > 0
                              then pp.start_date end) as r_90d_salable_days,
       count(distinct case
                          when pp.start_date >= curdate() - interval 30 day
                              and pp.afn_fulfillable_quantity > 0
                              then pp.start_date end) as r_30d_salable_days,
       count(distinct case
                          when pp.start_date >= curdate() - interval 14 day
                              and pp.afn_fulfillable_quantity > 0
                              then pp.start_date end) as r_14d_salable_days,
       count(distinct case
                          when pp.start_date >= curdate() - interval 7 day
                              and pp.afn_fulfillable_quantity > 0
                              then pp.start_date end) as r_7d_salable_days,
       count(distinct case
                          when pp.start_date >= curdate() - interval 3 day
                              and pp.afn_fulfillable_quantity > 0
                              then pp.start_date end) as r_3d_salable_days
from (select start_date,
      ...
      group by start_date,
               country_category,
               seller_name_new,
               seller_sku_adj) as pp
group by pp.country_category,
         pp.seller_name_new,
         pp.seller_sku_adj;
...
drop temporary table if exists opt_db.tmp_pur_plan_support_layer_all;
create temporary table opt_db.tmp_pur_plan_support_layer_all as
select pre.*,
       case
...
alter table opt_db.tmp_pur_plan_support_layer_all
    add index idx_tmp_pur_plan_support_layer_all (country_category(3), seller_name_new(64), seller_sku_adj(64), support_replenish_level_sort);

drop temporary table if exists opt_db.tmp_pur_plan_support_layer_summary;
create temporary table opt_db.tmp_pur_plan_support_layer_summary as
select support_replenish_level_sort,
       support_replenish_level,
       count(distinct concat_ws('|', country_category, seller_name_new, seller_sku_adj)) as msku_count,
       count(*) as row_count
from opt_db.tmp_pur_plan_support_layer_all
group by support_replenish_level_sort,
         support_replenish_level;
...
with listing数据 as (select new_old_product,
     ...
     补货建议数据 as (select *
                      ...
                      where dt_week = yearweek(date_sub(curdate(), interval 0 week), 1)),
     可售天数统计 as (select *
                      from opt_db.tmp_pur_plan_salable_days_current),
...
#                              end as sales_change_rate_adj
     ...
     联结 as (select wd.new_old_product,
                     ...
                     wd.support_inventory_qty,
                     wd.inventory_support_days,
                     wd.support_replenish_level,
                     wd.support_replenish_level_sort,
                            ...
                            case
                                when coalesce(history_recovery_flag, 0) = 1
                                 ...
                                 when ((replenish_comp_months * 30 * coalesce(daily_avg_sales, 0) - coalesce(available_total, 0) - coalesce(local_quantity, 0) - coalesce(stock_up_num, 0) - coalesce(sc_quantity_purchase_plan, 0))) > 0 and
                                      max_cg_box_pcs > 0
                                     then
                                    greatest(round(((replenish_comp_months * 30 * coalesce(daily_avg_sales, 0) - coalesce(available_total, 0) - coalesce(local_quantity, 0) - coalesce(stock_up_num, 0) - coalesce(sc_quantity_purchase_plan, 0))) *
                                           (sales_adj_factor) /
                                           max_cg_box_pcs,
                                           0), 1) * max_cg_box_pcs
                                 when ((replenish_comp_months * 30 * coalesce(daily_avg_sales, 0) - coalesce(available_total, 0) - coalesce(local_quantity, 0) - coalesce(stock_up_num, 0) - coalesce(sc_quantity_purchase_plan, 0))) > 0 and
                                      (max_cg_box_pcs = 0 or max_cg_box_pcs is null)
                                    then greatest(round(((replenish_comp_months * 30 * coalesce(daily_avg_sales, 0) - coalesce(available_total, 0) - coalesce(local_quantity, 0) - coalesce(stock_up_num, 0) - coalesce(sc_quantity_purchase_plan, 0))) *
                                               (sales_adj_factor), 0), 50)
                            ...
                            case
                                when coalesce(history_recovery_flag, 0) = 1
                                 ...
                                 when ((replenish_comp_months * 30 * coalesce(daily_avg_sales, 0) - coalesce(available_total, 0) - coalesce(local_quantity, 0) - coalesce(stock_up_num, 0) - coalesce(sc_quantity_purchase_plan, 0))) > 0 and
                                      max_cg_box_pcs > 0
                                     then
                                    greatest(round(((replenish_comp_months * 30 * coalesce(daily_avg_sales, 0) - coalesce(available_total, 0) - coalesce(stock_up_num, 0) - coalesce(local_quantity, 0) - coalesce(sc_quantity_purchase_plan, 0))) *
                                           (sales_adj_factor) /
                                           max_cg_box_pcs, 0), 1)
